...
async function getSongs(folder) {
    currFolder = folder
    let song_list = await fetch(`http://192.168.215.57:3000/${folder}/`)
    ...
    for (let index = 0; index < song_links.length; index++) {
     ...
     for (const song of songs) {
         ...
         <div class="song-info">
             ...
             <div class="song-name-play-text">${decodeURI(song).replaceAll("-", " ").split("/")[1]}</div>
         </div>
         <div class="play-now">
         <span>Plan Now</span>
...
const playMusic = (track, pause = false) => {
    ...
    var currentSongName = (decodeURI(track).replaceAll("-", " ").replaceAll(".mp3", "").replaceAll("%20", " ").slice(0, 60).split("/")[1])
